...
class FusedSigmoidGatingPerHeadTest(unittest.TestCase):
    ...
    def test_verify_writes_every_chained_checkpoint_to_strided_spec_pages(self):
        """A multi-step verify equals chained single-token recurrent calls.

        Every request/step owns a distinct spec page.  This catches flattening
        ``[shard, request, step]`` as ``[shard, request]`` and also exercises
        two non-contiguous state shards.  The canonical pages are deliberately
        absent from the call and must remain byte-identical.
        """
        torch = self.torch
        batch, steps = 3, 4
        inp = self._inputs()
        total = batch * steps
        q = torch.randn(1, total, self.H, self.K, device="cuda", dtype=torch.bfloat16)
        k = torch.randn(1, total, self.H, self.K, device="cuda", dtype=torch.bfloat16)
        v = torch.randn(1, total, self.HV, self.V, device="cuda", dtype=torch.bfloat16)
        a = torch.randn(total, self.HV, device="cuda", dtype=torch.float32)
        b = torch.randn(total, self.HV, device="cuda", dtype=torch.float32)
        cu = torch.arange(0, total + 1, steps, dtype=torch.int32, device="cuda")
        in_pages = torch.tensor(
            [[1, 2, -1], [2, 1, -1]], dtype=torch.int32, device="cuda"
        )
        ...
        spec_storage = torch.full(
            (self.KSHARDS, batch, steps * 2),
            -1,
            dtype=torch.int32,
            device="cuda",
        )
        spec_pages = spec_storage[..., ::2]
        spec_pages.copy_(spec_values)
        self.assertFalse(spec_pages.is_contiguous())
        self.assertEqual(spec_pages.stride(2), 2)
        canonical_pages = torch.tensor(
            [
                [[21, 21, 22, 22], [23, 23, 23, 24], [-1, -1, -1, -1]],
                [[25, 25, 26, 26], [27, 27, 27, 28], [-1, -1, -1, -1]],
            ],
            dtype=torch.int32,
            device="cuda",
        )

        # Expand the page count used by this test while retaining the real
        # padded page-row stride of _make_shards.
        h0 = torch.randn(30, self.HV, self.K, self.V, device="cuda")
        slabs_verify, _ = self._make_shards(h0)
        slabs_ref, _ = self._make_shards(h0)
        verify_before = [slab.clone() for slab in slabs_verify]
        canonical_before = [
            slab[canonical_pages[s][canonical_pages[s] >= 0].unique(), 0].clone()
            for s, slab in enumerate(slabs_verify)
        ]
        head_base, head_shard, prs = self._head_maps(slabs_verify)

        verify_inp = SimpleNamespace(
            q=q,
            k=k,
            v=v,
            a=a,
        ...
        for step in range(steps):
            token_ids = torch.arange(batch, device="cuda") * steps + step
            step_inp = SimpleNamespace(
                q=q[:, token_ids],
                k=k[:, token_ids],
                v=v[:, token_ids],
                a=a[token_ids],
                ...
                A_log=inp.A_log,
                dt_bias=inp.dt_bias,
                cu=torch.arange(batch + 1, dtype=torch.int32, device="cuda"),
            )
            out_steps.append(
                self._per_head(
                    step_inp,
                    slabs_ref[0],
                    *self._head_maps(slabs_ref),
                    state_in_pages=ref_in,
                    state_out_pages=spec_pages[:, :, step],
                    disable_state_update=True,
                )
            )
            # state_in_pages has a compact [shard, request] ABI; the tested
            # output table remains non-contiguous, but each chained reference
            # step must compact its selected column before using it as input.
            ref_in = spec_pages[:, :, step].contiguous()

        out_ref = torch.stack(out_steps, dim=2)
        out_verify_by_req = out_verify.view(1, batch, steps, self.HV, self.V)
        # The padded row's output is scrubbed by the caller and is irrelevant;
        # real requests must match the chained recurrence bit-for-bit.
        self.assertTrue(torch.equal(out_verify_by_req[:, :2], out_ref[:, :2]))
        for shard, (verify, ref) in enumerate(zip(slabs_verify, slabs_ref)):
            real_spec = spec_pages[shard][spec_pages[shard] >= 0]
            canonical = canonical_pages[shard][canonical_pages[shard] >= 0].unique()
            self.assertTrue(torch.equal(verify[real_spec, 0], ref[real_spec, 0]))
            self.assertTrue(
                torch.equal(
                    verify[canonical, 0],
                    canonical_before[shard],
                )
            )
            self.assertEqual(verify[:, 1].abs().max().item(), 0.0)
            untouched = torch.ones(verify.shape[0], dtype=torch.bool, device="cuda")
            untouched[real_spec] = False
            self.assertTrue(
                torch.equal(verify[untouched, 0], verify_before[shard][untouched, 0])
            )

    def test_per_head_2d_output_pages_reject_multi_token_sequences(self):
        """A 2-D table cannot represent one destination per verify step."""
        torch = self.torch
        inp = self._inputs()
        slabs, _ = self._make_shards(inp.h0)
        in_pages = torch.tensor([[1, 2], [2, 1]], dtype=torch.int32, device="cuda")
        out_pages = torch.tensor([[4, 5], [6, 7]], dtype=torch.int32, device="cuda")
        # Four packed tokens split into two requests => two steps/request.
        inp.cu = torch.tensor([0, 2, 4], dtype=torch.int32, device="cuda")

        with self.assertRaisesRegex(ValueError, "multi-token verify"):
            self._per_head(
                inp,
                slabs[0],
    ...
    def test_per_head_noncontiguous_input_pages_are_rejected(self):
        """The input-page kernel ABI advances requests with ``+ i_n``."""
        torch = self.torch
        inp = self._inputs()
        slabs, _ = self._make_shards(inp.h0)
        in_storage = torch.ones(
            self.KSHARDS, self.B * 2, dtype=torch.int32, device="cuda"
        )
        in_pages = in_storage[:, ::2]
        self.assertEqual(in_pages.stride(1), 2)
        out_pages = torch.ones(self.KSHARDS, self.B, dtype=torch.int32, device="cuda")

        with self.assertRaisesRegex(ValueError, "contiguous in the request"):
            self._per_head(
                inp,
                slabs[0],
                *self._head_maps(slabs),
